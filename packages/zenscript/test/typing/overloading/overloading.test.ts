...
describe('check overload', async () => {
  ...
  suite('overload_basic', () => {
    ...
    it('varargs', () => {
      ...
      expect(varargs_low_priorty.params.length).toBe(1)
      ...
      expect(varargs_full.params.length).toBe(2)
      expect(varargs_full.params[1].varargs).toBe(true)
      ...
      expect(varargs_more.params.length).toBe(2)
      expect(varargs_more.params[1].varargs).toBe(true)
      ...
      expect(varargs_less.params.length).toBe(1)
      expect(varargs_less.params[0].varargs).toBe(true)
    })
    ...
    it('optional', () => {
      ...
      expect(optional_eq.params.length).toBe(2)
      expect(optional_eq.params[1].defaultValue).toBeDefined()
      ...
      expect(optional_low_priorty.params.length).toBe(1)
      ...
      expect(optional_less.params.length).toBe(1)
      expect(optional_less.params[0].defaultValue).toBeDefined()
      ...
      expect(optional_convert_1.params.length).toBe(2)
      expect(optional_convert_1.params[1].defaultValue).toBeDefined()
      ...
      expect(optional_convert_2.params.length).toBe(2)
      expect(optional_convert_2.params[1].defaultValue).toBeDefined()
      ...
      expect(optional_vs_varargs.params.length).toBe(1)
      expect(optional_vs_varargs.params[0].varargs).toBeTruthy()
      expect(optional_vs_varargs.params[0].defaultValue).toBeFalsy()
    })
  })
})
...
describe('check static overload', async () => {
  ...
  it('import overload', () => {
    ...
    assert(isFunctionDeclaration(foo1))
    expect(foo1.name).toBe('foo')
    expect(foo1.params.length).toBe(0)

    const foo2 = findOverloadForCall(script_overload_zs.statements[1])
    assert(isFunctionDeclaration(foo2))
    expect(foo2.name).toBe('foo')
    expect(foo2.params.length).toBe(1)
  })
